...
/**
 * @ClassName WordUpdate (对word中特定的字符串进行替换)
 * @Author panha
 * @Date 2019/5/13 19:36
 * @Version 1.0
 **/
public class WordUpdate {
    private static final Logger logger = LoggerFactory.getLogger(WordUpdate.class);
    /**
...
//            }
    ...
    public static String UpdateWord(String url,String returnUrl,Map<String,String> params){
        OutputStream out = null;
        try {
            logger.info("======>更新word開始，url: " + url + ", return url: " + returnUrl);
            XWPFDocument document = new XWPFDocument(new FileInputStream(url));
            searchAndReplace(document, params);
            out = new FileOutputStream(returnUrl);
            document.write(out);
            logger.info("======>更新word結束");
            return returnUrl;
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
