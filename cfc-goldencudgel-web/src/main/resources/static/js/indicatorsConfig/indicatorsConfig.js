...
function  queryRatingconfig(curPage) {
    queryDataCommon(curPage,"/indicatorsConfig/queryIndicatorsConfig","indicators_list");
}

function  viewIncomDetail(obj) {
    var configid = $(obj).attr("compilationNo");
    window.location.href = context_path + "/indicatorsConfig/seeIndicators?configid="+configid+getSearchData();
}

function editIncom(obj) {
    var configid = $(obj).attr("compilationNo");
    window.location.href = context_path + "/indicatorsConfig/editIndicators?configid="+configid+getSearchData();
}
