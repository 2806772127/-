...
(function($, undefined) {
  ...
  mouseProto._mouseInit = function() {
    ...
    this._mouseMoveDelegate = function(event) {
      if (gesturesSupport && event.originalEvent.touches
              && event.originalEvent.touches.length > 1) { return; }
  ...
  /**
   * Simple implementation of jQuery like getters/setters var val = something();
   * something(val);
   */
  ...
  // this test is the inversion of the css filters test from the modernizr
  // project
  useIeTransforms = function() {
    var modElem = document.createElement('modernizr'), mStyle = modElem.style, omPrefixes = 'Webkit Moz O ms', domPrefixes = omPrefixes
            .toLowerCase().split(' '), props = ("transform" + ' '
            + domPrefixes.join("Transform ") + "Transform").split(' ');
    /* using 'for' loop instead of 'for in' to avoid issues in IE8 */
    for (var i = 0; i < props.length; i++) {
      var prop = props[i];
      if (prop.indexOf("-") == -1 && mStyle[prop] !== undefined) { return false; }
    }
    return true;
  }();
  ...
  $
          .widget(
                  ...
                  $.ui.mouse,
                  {
                    ...
                    options: {
                      /**
                       * start zoom value for image, not used now may be equal
                       * to "fit" to fit image into container or scale in %
                       */
                      zoom: "fit",
                      /**
                      ...
                      zoom_min: 25,
                      /**
                       * base of rate multiplier. zoom is calculated by formula:
                       * zoom_base * zoom_delta^rate
                       */
                      zoom_delta: 1.4,
                      /**
                      ...
                      mousewheel: true,
                      /**
                       * if false, plugin doesn't bind resize event on window
                       * and this must be handled manually
                       */
                      ...
                      /**
                       * if true the image will fill the container and the image
                       * will be distorted
                       */
                      ...
                      onZoom: jQuery.noop,
                      /**
                       * event is triggered when zoom value is changed after
                       * image is set to the new dimensions
                       * 
                       * @param int
                       *          new zoom value
                       * @return boolean if false zoom action is aborted
                       ...
                       * 
                       * @param object
                       *          coords mouse coordinates on the image
                       * @return boolean if false is returned, drag action is
                       *         aborted
                       */
                      ...
                      onClick: jQuery.noop,
                      /**
                       * mouse double click event. If used will delay each click
                       * event. If double click event was fired, clicks will
                       * not.
                       * 
                       * @param object
                       *          coords mouse coordinates on the image
                      ...
                      onStartLoad: null,
                      /**
                       * event is fired, when image is loaded and initially
                       * positioned
                       */
                      onFinishLoad: null,
                      /**
                    ...
                    _create: function() {
                      ...
                      var useDblClick = !!this.options.onDblClick
                              || this.options.zoom_on_dblclick, dblClickTimer = null, clicksNumber = 0;
                    ...
                    destroy: function() {
                      ...
                      this.container.css('overflow', ''); // cleanup styles on
                                                          // destroy
                    },
                     ...
                     * @param {boolean}
                     *          isActive Sample :
                     *          $('#viewer').iviewer('activateMouseWheel',
                     *          true);
                     */
                    activateMouseWheel: function(isActive) {
                      ...
                      if (gesturesSupport) {
                        this.img_object.object().unbind('touchstart').unbind(
                                'gesturechange.iviewer').unbind(
                                'gestureend.iviewer');
                      }
                      ...
                      if (isActive) {
                        ...
                        this.container
                                .bind(
                                        'mousewheel.iviewer',
                                        function(ev, delta) {
                                          // this event is there instead of
                                          // containing div, because
                                          // at opera it triggers many times on
                                          // div
                                          var zoom = (delta > 0) ? 1 : -1, container_offset = me.container
                                                  .offset(), mouse_pos = {
                                            // jquery.mousewheel 3.1.0 uses
                                            // strange MozMousePixelScroll event
                                            // which is not being fixed by
                                            // jQuery.Event
                                            x: (ev.pageX || ev.originalEvent.pageX)
                                                    - container_offset.left,
                                            y: (ev.pageY || ev.originalEvent.pageX)
                        ...
                        if (gesturesSupport) {
                          ...
                          this.img_object
                                  ...
                                  .bind(
                                          'gesturechange.iviewer',
                                          function(ev) {
                                            // do not want to import throttle
                                            // function from underscore
                                            var d = +new Date();
                                            if ((d - gestureThrottle) < 50) { return; }
                                            gestureThrottle = d;
                                            var zoom = originalScale
                                                    * ev.originalEvent.scale;
                                            me.set_zoom(zoom, originalCenter);
                                            ev.preventDefault();
                                          }).bind('gestureend.iviewer',
                                          function(ev) {
                                            originalCenter = null;
                                          });
                        }
                    ...
                    fit: function(skip_animation) {
                      ...
                      var window_ratio = this.options.width
                              / this.options.height;
                      var choose_left = (aspect_ratio > window_ratio);
                      var new_zoom = 0;

                      if (choose_left) {
                        new_zoom = this.options.width
                                / this.img_object.orig_width() * 100;
                      } else {
                        new_zoom = this.options.height
                                / this.img_object.orig_height() * 100;
                      }
                    ...
                    center: function() {
                      this
                              .setCoords(
                                      -Math
                                              .round((this.img_object
                                                      .display_width() - this.options.width) / 2),
                                      -Math
                                              .round((this.img_object
                                                      .display_height() - this.options.height) / 2));
                    },
                    ...
                    _correctCoords: function(x, y) {
                      ...
                      if (y + this.img_object.display_height() < this.options.height) {
                        y = this.options.height
                                - this.img_object.display_height();
                      }
                      if (x + this.img_object.display_width() < this.options.width) {
                        x = this.options.width
                                - this.img_object.display_width();
                      }
                    ...
                    /**
                     * convert coordinates on the container to the coordinates
                     * on the image (in original size)
                     * 
                     * @return object with fields x,y according to coordinates
                     *         or false if initial coords are not inside image
                     */
                    ...
                    /**
                     * convert coordinates on the image (in original size, and
                     * zero angle) to the coordinates on the container
                     * 
                     * @return object with fields x,y according to coordinates
                     */
                     ...
                     * 
                     * @param e -
                     *          object containing pageX and pageY fields, e.g.
                     *          mouse event object
                     * @return object with fields x,y according to coordinates
                     *         or false if initial coords are not inside image
                     */
                    _getMouseCoords: function(e) {
                      var containerOffset = this.container.offset(), coords = this
                              .containerToImage(e.pageX - containerOffset.left,
                     ...
                     * 
                     * @param {boolean}
                     *          fill wether to fill the container entirely or
                     *          not.
                     */
                    fill_container: function(fill) {
                      ...
                      if (fill) {
                        ...
                        if (ratio > 1)
                          this.img_object.orig_width(this.img_object
                                  .orig_height()
                                  * ratio);
                        else
                          this.img_object.orig_height(this.img_object
                                  .orig_width()
                                  * ratio);
                      } else {
                        this.img_object
                                .orig_width(this._fill_orig_dimensions.width);
                        this.img_object
                                .orig_height(this._fill_orig_dimensions.height);
                      }
                     ...
                     * 
                     * @param {number}
                     *          new_zoom image scale in %
                     * @param {boolean}
                     *          skip_animation
                     * @param {x:
                     *          number, y: number} Coordinates of point the
                     *          should not be moved on zoom. The default is the
                     *          center of image.
                     */
                    set_zoom: function(new_zoom, skip_animation, zoom_center) {
                      ...
                      if (this.current_zoom == "fit") {
                        old_x = zoom_center.x
                                + Math.round(this.img_object.orig_width() / 2);
                        old_y = zoom_center.y
                                + Math.round(this.img_object.orig_height() / 2);
                      ...
                      var new_width = util.scaleValue(this.img_object
                              .orig_width(), new_zoom);
                      var new_height = util.scaleValue(this.img_object
                              .orig_height(), new_zoom);
                      var new_x = util.scaleValue(util.descaleValue(old_x,
                      ...
                      this.img_object.setImageProps(new_width, new_height,
                              coords.x, coords.y, skip_animation, function() {
                    ...
                    /**
                     * shows or hides the controls controls are shown/hidden
                     * based on user input
                     * 
                     * @param Boolean
                     *          flag that specifies whether to show or hide the
                     *          controls
                     */
                    ...
                    /**
                     * changes zoom scale by delta zoom is calculated by
                     * formula: zoom_base * zoom_delta^rate
                     * 
                     * @param Integer
                     *          delta number to add to the current multiplier
                     *          rate number
                     * @param {x:
                     *          number, y: number=} Coordinates of point the
                     *          should not be moved on zoom.
                     */
                    zoom_by: function(delta, zoom_center) {
                      var closest_rate = this
                              .find_closest_zoom_rate(this.current_zoom);

                      var next_rate = closest_rate + delta;
                      var next_zoom = this.options.zoom_base
                     ...
                     * 
                     * @param {num}
                     *          deg Degrees amount to rotate. Positive values
                     *          rotate image clockwise. Currently 0, 90, 180,
                     *          270 and -90, -180, -270 values are supported
                     * @param {boolean}
                     *          abs If the flag is true if, the deg parameter
                     *          will be considered as a absolute value and
                     *          relative otherwise.
                     * @return {num|null} Method will return current image angle
                     *         if called without any arguments.
                     */
                    angle: function(deg, abs) {
                      if (arguments.length === 0) { return this.img_object
                              .angle(); }
                      ...
                      // the rotate behavior is different in all editors. For
                      // now we just center the
                      // image. However, it will be better to try to keep the
                      // position.
                      this.center();
                    ...
                    /**
                     * finds closest multiplier rate for value basing on
                     * zoom_base and zoom_delta values from settings
                     * 
                     * @param Number
                     *          value zoom value to examine
                     */
                    find_closest_zoom_rate: function(value) {
                      ...
                      while (Math.abs(func(this.options.zoom_base, Math.pow(
                              mltplr, rate))
                              - value) > Math.abs(func(this.options.zoom_base,
                    ...
                    /**
                     * Get some information about the image. Currently
                     * orig_(width|height), display_(width|height), angle, zoom
                     * and src params are supported.
                     * 
                     * @param {string}
                     *          parameter to check
                     * @param {boolean}
                     *          withoutRotation if param is orig_width or
                     *          orig_height and this flag is set to true, method
                     *          will return original image width without
                      ...
                      case 'orig_height':
                        if (withoutRotation) {
                          return (this.img_object.angle() % 180 === 0
                                  ...
                                  : param === 'orig_width' ? this.img_object
                                          .orig_height() : this.img_object
                                          .orig_width());
                        } else {
                    ...
                    /**
                     * callback for handling mousdown event to start dragging
                     * image
                     */
                    _mouseStart: function(e) {
                      $.ui.mouse.prototype._mouseStart.call(this, e);
                      if (this._trigger('onStartDrag', 0, this
                              ._getMouseCoords(e)) === false) { return false; }
                    ...
                    /**
                     * Handle mouse move if needed. User can avoid using this
                     * callback, because he can get the same information through
                     * public methods.
                     * 
                     * @param {jQuery.Event}
                     *          e
                    ...
                    _mouseDrag: function(e) {
                      ...
                      // #10: imitate mouseStart, because we can get here
                      // without it on iPad for some reason
                    ...
                    _dblclick: function(ev) {
                      if (this.options.onDblClick) {
                        this
                                ._trigger('onDblClick', 0, this
                                        ._getMouseCoords(ev));
                      }

                      if (this.options.zoom_on_dblclick) {
                        var container_offset = this.container.offset(), mouse_pos = {
                    ...
                    createui: function() {
                      ...
                      this.zoom_object = $("<div>").addClass(
                              "iviewer_zoom_status iviewer_common").appendTo(
                              this.container);
  ...
  /**
   * @class $.ui.iviewer.ImageObject Class represents image and provides public
   *        api without extending image prototype.
   * @constructor
   * @param {boolean}
   *          do_anim Do we want to animate image on dimension changes?
   */
  ...
  (function() {
     ...
     *          src Image url.
     * @param {Function=}
     *          loaded Function will be called on image load.
     */
    this.load = function(src, loaded, error) {
      ...
      // #67: don't use image object for loading in case naturalWidth is
      // supported
      // because later assigning to self._img[0] may result in additional image
      // requesrts.
      ...
      img.onload = function() {
        ...
        // max-width is reset, because plugin breaks in the twitter bootstrap
        // otherwise
        .css({
          position: "absolute",
          top: "0px",
    ...
    this._dimension = function(prefix, name) {
      var horiz = '_' + prefix + '_' + name, vert = '_' + prefix + '_'
              + (name === 'height' ? 'width' : 'height');
    ...
    /**
     * Getters and setter for common image dimensions. display_ means real image
     * tag dimensions orig_ means physical image dimensions. Note, that
     * dimensions are swapped if image is rotated. It necessary, because as
     * little as possible code should know about rotation.
     */
    ...
    this.orig_width = this._dimension('orig', 'width'), this.orig_height = this
            ._dimension('orig', 'height'),

    /**
     * Setter for X coordinate. If image is rotated we need to additionaly shift
     * an image to map image coordinate to the visual position.
     * 
     * @param {number}
     *          val Coordinate value.
     * @param {boolean}
     *          skipCss If true, we only set the value and do not touch the dom.
     */
    this.x = setter(function(val, skipCss) {
      this._x = val;
      if (!skipCss) {
        this._finishAnimation();
        this._img.css("left", this._x
                + (this._swapDimensions ? this.display_diff() / 2 : 0) + "px");
      }
    }, function() {
      return this._x;
    });
    ...
    /**
     * Setter for Y coordinate. If image is rotated we need to additionaly shift
     * an image to map image coordinate to the visual position.
     * 
     * @param {number}
     *          val Coordinate value.
     ...
     * 
     * @param {number}
     *          deg Absolute image angle. The method will work with values 0,
     *          90, 180, 270 degrees.
     */
     ...
     * You will get coordinates of point on image with respect to rotation, but
     * will be set as if image was not rotated. So, if image was rotated 90
     * degrees, it's (0,0) point will be on the top right corner.
     * 
     * @param {{x:
     *          number, y: number}} point Point in container coordinates.
     ...
     *          y
     * @param {boolean}
     *          skip_animation If true, the animation will be skiped despite the
     *          value set in constructor.
     * @param {Function=}
     *          complete Call back will be fired when zoom will be complete.
     */
    this.setImageProps = function(disp_w, disp_h, x, y, skip_animation,
            complete) {
      ...
      if (useIeTransforms) {
        jQuery.extend(params, {
          marginLeft: ieTransforms[this.angle()].marginLeft
                  * this.display_diff() / 2,
          marginTop: ieTransforms[this.angle()].marginTop * this.display_diff()
                  / 2
        });
      }
      ...
      // here we come: another IE oddness. If image is rotated 90 degrees with a
      // filter, than
      // width and height getters return real width and height of rotated image.
      // The bad news
      ...
      if (useIeTransforms && swapDims) {
        var ieh = this._img.width(), iew = this._img.height(), iedh = params.height
                - ieh;
    ...
    // if we set image coordinates we need to be sure that no animation is
    // active atm
